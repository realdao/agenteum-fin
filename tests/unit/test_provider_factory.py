...
def test_build_services_allows_disabling_optional_providers():
    services = build_services(
        Settings(
            ...
            AGENTEUM_FIN_FUNDAMENTAL_PROFILE_PROVIDER="none",
            AGENTEUM_FIN_FUNDAMENTAL_FINANCIALS_PROVIDER="none",
            AGENTEUM_FIN_ANNOUNCEMENTS_PROVIDER="none",
        )
    )

    assert services.kline_service.a_share_provider is None
    assert services.snapshot_service.profile_provider is None
    assert services.snapshot_service.financials_provider is None
    assert services.announcement_service.provider is None


def test_build_services_shares_quote_provider_between_profile_and_snapshot():
    services = build_services(Settings())

    assert services.snapshot_service.quote_provider is services.profile_service.providers[0]


def test_build_services_rejects_unknown_fundamental_providers():
    with pytest.raises(ProviderError) as raised:
        build_services(Settings(AGENTEUM_FIN_FUNDAMENTAL_PROFILE_PROVIDER="unknown"))
    assert raised.value.error_type == ErrorType.CONFIG_ERROR
    assert raised.value.provider == "fundamental_profile"

    with pytest.raises(ProviderError) as raised:
        build_services(Settings(AGENTEUM_FIN_FUNDAMENTAL_FINANCIALS_PROVIDER="unknown"))
    assert raised.value.error_type == ErrorType.CONFIG_ERROR
    assert raised.value.provider == "fundamental_financials"

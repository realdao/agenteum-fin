...
- `stock_kline` asset coverage: the default tencent provider serves stocks, on-exchange funds (ETF/LOF), and indices (SZ `399xxx` bare; SH `000xxx` with explicit `.SH` suffix - bare `000xxx` resolves to Shenzhen stocks for backward compatibility). mootdx serves stocks only. All other stock_* tools (`stock_profile`, `stock_financial_statements`, `stock_fundamental_snapshot`, `stock_announcements`) accept stocks only and reject fund/index symbols with a routing hint.

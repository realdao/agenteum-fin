...
- **场内基金（ETF/LOF）或指数，要 K 线** → 同样用 `stock_kline`：沪市基金 50/51/56/58 开头、深市基金 15/16/18 开头（如 561360、159870）；深证/国证指数用 399xxx 裸码（如 399365），上证系列指数必须显式 `.SH` 后缀（如 `000001.SH` 上证指数、`000300.SH` 沪深300；裸码 000001 是平安银行）。指数仅支持 `adjust=none`。**基金的净值/涨跌幅/跟踪指数等非 K 线数据**仍走 `iwencai_query`（domain=market）。
...
- **场内基金（ETF/LOF）的非 K 线数据一律走本工具**（domain=market），stock_profile/snapshot/三表等个股工具会拒绝基金代码并返回路由提示；基金 K 线可用 stock_kline。
- **实体一律用纯代码**：股票/ETF 用 6 位代码（159698），指数用指数代码（399365），避免"简称+代码"混合写法和纯中文名称。中文名称（如"国证粮食指数"）可能被 NLU 误拆成行业条件（实测被拆为"所属同花顺行业包含粮食"+"沪深300权重排名"导致空结果）。
- **控制查询复杂度：单查询 ≤2 个实体、≤4 个字段**。多实体×多字段的笛卡尔积超出网关组装能力时会**静默丢行**（实测 3 只 ETF×5 字段只返回 1 只且无报错）。响应出现 `partial_data_warning` 字段时必须拆成单实体查询补齐，勿用残缺数据下结论。
- **价格/走势类查询显式指定频率与区间**（如"2026年6月以来月度"），防止网关自选日度全量数据淹没上下文（实测小麦日度数据约 200 行、同一查询里玉米却是月末值，颗粒度不一致）。响应出现 `data_volume_tip` 说明网关返回超量，请加显式频率/区间后重查。
- **域边界与直觉不一致时，换域+换写法重试**：指数成分股在 index 域查不到，但在 astock 域用指数代码可查到（实测"399365的成分股" domain=astock 成功）。重要查询失败后优先换域，而不是原样放宽条件。指数成分的权重字段网关不提供，需接受或人工补充。
- **字段名逐个核对口径后再引用**：网关字段名是缩写，掩盖真实口径（实测 `最低价[20260814]` 实为区间最低价而非当日最低）。引用前确认区间、复权、单位、截止日期。
...
> 局限：**指数成分股/权重不在此域**，用 astock 域 + 指数代码（如 `399365的成分股`）；指数 K 线用 `stock_kline`（399xxx 裸码、000xxx.SH 显式后缀）。
...
| ETF 行情/净值/跟踪指数 | `iwencai_query` domain=`market` | `561360、159670最新价格和涨跌幅` |
| ETF/指数 K 线 | `stock_kline` | `561360`、`399365`、`000300.SH` |
| 指数成分股 | `iwencai_query` domain=`astock` | `399365的成分股` |
...
- **部分失败告警**：`has_more` 为 `false` 但本页累计返回数（`(page-1)*limit + returned_count`）小于 `code_count` 时，响应附带 `partial_data_warning`——网关静默丢弃了部分结果。此时必须核对请求的实体数，将缺失实体拆成单独查询补齐，勿直接基于残缺数据下结论。
- **超量提示**：`returned_count` 超过请求的 `limit` 时响应附带 `data_volume_tip`——网关把 limit 当实体数、时间序列行数不受控。若仅需趋势，请在 query 里显式指定频率与区间（如"2026年6月以来月度"）后重查。
